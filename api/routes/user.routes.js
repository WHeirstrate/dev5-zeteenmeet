 ...
 * @returns {JSON} - A JSON-object with all users will be returned.
 */
  ...
  if (req.body.email && req.body.password) {
    try {
      PG("users")
        ...
        .returning("*")
        .then((data) => {
          if (!data.length) return res.status(400).send("User does not exist");
...
USERROUTER.route("/login").post((req, res) => loginUser(req, res));
USERROUTER.route("/:id")
